 ...
 * exactly as the Builder's Add Futures dialog places it (HC-TR-035; GAPS #74). Lives in the pricing package (ADR-077)
 * so the API's backtest and the web app place the same templates the same way; the asset type is generic because
 * this package does not depend on the schema package.
 */
...
export interface MaterialiseInput<A extends string = string> {
  asset: A;
...
/** A leg a template produced on the ladder: the shape the web app adds to a strategy and the backtest prices. */
export interface MaterialisedLeg<A extends string = string> {
  asset: A;
  kind: LegKind;
  side: Side;
  strike: string;
...
export type MaterialiseResult<A extends string = string> = { ok: true; legs: MaterialisedLeg<A>[] } | { ok: false; reason: "no-chain" | "out-of-range" | "no-quote" | "no-spot" };
 ...
 */
export function materialiseTemplate<A extends string>(tpl: StrategyTemplate, input: MaterialiseInput<A>): MaterialiseResult<A> {
  const legs: MaterialisedLeg<A>[] = [];
