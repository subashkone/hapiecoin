// The template catalogue and its placement on a ladder (HC-TR-037..040; ADR-077 moved the core here from the web app).
import { describe, expect, it } from "vitest";
import { type StrategyTemplate, TEMPLATES, TEMPLATE_CATEGORIES, TEMPLATE_COUNT, isTemplateName, materialiseTemplate, templateByName } from "./templates.js";
...
describe("HC-TR-155 isTemplateName", () => {
  it("matches a catalogue name in any case and refuses blanks and made-up names", () => {
    expect(isTemplateName(" buy call ")).toBe(true);
    expect(isTemplateName("")).toBe(false);
    expect(isTemplateName("My own thing")).toBe(false);
  });
});

describe("HC-TR-040 a calendar with no listed expiries falls back to the placed expiry", () => {
  it("prices both legs off the near chain when the expiry list is empty", () => {
    const calendar = templateByName("Long Calendar with Calls")!;
    const r = materialiseTemplate(calendar, { ...base, expiries: [] });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.legs.map((l) => l.expiry)).toEqual([base.expiry, base.expiry]);
  });
});
