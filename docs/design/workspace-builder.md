...
## Amendment · share links, greeks across price, deep links, collapse, palette (10 Sep 2026, ADR-049)
Pane header gains `pane-strategy-info` ("Bull Call Spread · 25 Sep · 20 lots"), **↗ Share** (`share-open` → `ShareDialog`: link `share-link`, `share-copy`, legs and figures) and **›** (`collapse-right`); the left tab bar shows `Lot 0.001 BTC · basis mark` (`left-tab-info`) and **‹** (`collapse-left`); a collapsed workspace carries `data-collapse` and a ⋮ `collapse-restore` handle. `/analyse?tab=paper&panel=ladder` selects the tabs on arrival. Greeks: vega in currency, `chart-delta-price` and `chart-theta-price` across ±20 % on the target date. Ladder step persists (`store.ladderStep`). `/s/<code>` (`components/share/ShareLanding.tsx`) decodes, loads and redirects. Palette: "Switch expiry → date" per listed expiry, "Load template → name" per template (`store.templateRequest` consumed by `useTemplateLoader`).
