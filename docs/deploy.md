...
| api | `@hapiecoin/api` (Hono) | 3001 | `DATABASE_URL`, `REDIS_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL` / `WEB_URL`, `CREDENTIALS_ENC_KEY` (+ `CREDENTIALS_ENC_KEYS_PREVIOUS` after a rotation, ADR-054), `RESEND_API_KEY`, `EMAIL_FROM`, `RAZORPAY_*`, `GOOGLE_CLIENT_*`, `TRUSTED_PROXY_IPS`, `EGRESS_IP`, `DELTA_*_URL` | `apps/api/Dockerfile` (403 MB, runs migrations at boot). Refuses production without `DATABASE_URL` and `RESEND_API_KEY`. Reconciler runs inside the process (ADR-029). |
