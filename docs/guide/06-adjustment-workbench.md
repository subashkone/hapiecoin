...
Part of the [HapieCoin feature guide](README.md). 12 traced features on 4 screens; 12 built and tested, 0 still mock-only (listed in the backlog).
...
| HC-TR-154 | Repairs offered from the workbench (was: three quick fixes), each a draft loaded for review | Superseded on 20 Sep 2026 by the repair ideas (HC-TR-194 the diagnosis, HC-TR-195 the catalogue and goals, ADR-094): the three position-blind buttons (roll strikes up, roll out, hedge with a call) are gone; every idea is still built from the open legs and the listed strikes, priced with the engine, tagged with facts, loads a draft for review, and says why when it cannot be built | built | unit (web) |
| HC-TR-194 | Diagnosis line: how far spot is from each short strike, which side is tested, whether the loss has a limit | diagnose() measures every short contract on the shown expiry from spot in percent and in expected moves (the engine's one-sigma figure), counts twin rows on one contract as one short, names the nearest short as the tested side and marks it under pressure when it is in the money or under one expected move away; the panel shows the sentence and, in red, 'the loss has no limit' when the position's max loss is unbounded; a long-only position reads 'no short option on this expiry'; before the spot price arrives it reads 'Waiting for the spot price' and no idea is offered | built | unit (web) · e2e (Playwright) |
| HC-TR-195 | Repair ideas: wings by price, rolls, sizing, each priced before → after and ordered by the trader's goal | repairIdeas() builds drafts from the whole listed ladder only: cap the risk with cheap / balanced / tight wings (nearest strike whose mark is within 10 / 25 / 50 % of the short's mark; past the ladder's end the furthest listed strike with a note; sized by the short lots no long of that kind answers; duplicates dropped), roll the tested short two strikes away, roll the untested side in while it keeps half an expected move of room, roll out, roll out and away, halve or close the tested short (twin rows together); long-only: convert to a spread, roll to a cheaper strike; a zero or missing quote is never a price; an order that would meet the other side of its own contract or carry a short onto or past its long makes the idea unavailable with the reason; on a roll out a short never lands nearer spot and the card says when a strike moved. Each card shows the orders in words, before → after max loss, max profit, break-evens, POP, delta, cash now, the order count and what the idea gives up; before → after compares the whole trade from entry, so a close carries the result it locks in (realisedLegs, also in the pane's working change and the plans); goals under 'Order by': reduce max loss, collect credit, get neutral, widen break-evens (default from the diagnosis); tags are facts ('defines your risk', one leader per goal, 'largest credit' only for cash received); Load fills the working change through applyDraft and asks once before replacing a hand-built change, nothing is sent before Review; the panel sits under the position ticket, is built and priced on a market copy that settles every two seconds and re-orders only when the figures are quiet | built | unit (web) · e2e (Playwright) |
