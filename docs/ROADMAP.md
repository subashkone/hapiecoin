...
2. 🔨 Overlap line in previews and workbench Review · drift check vs. the exchange · out-of-sync badge · Reconcile (HC-TR-159..161; GAPS #67 partly) · deferred: server-side drift pass, Re-enter
...
24. ⬜ `.github/workflows/ci.yml` (`pnpm ci` + `check:budget` + e2e), dependabot, LICENSE (GAPS #69)
25. ⬜ API `bodyLimit` and a per-route limiter on `/live/place`, `/live/batch`, `/live/positions/exit` (GAPS #70)
