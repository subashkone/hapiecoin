...
9. 🔨 Backtest and replay on our own IV / mark snapshots: B9a end-of-day chain table (chain_eod, once a day at the settlement hour, a week backfilled from the mark stream) + the pricing package's runBacktest (a template entered every recorded day, settled at intrinsic, open trades marked or labelled modelled) + GET /v1/backtest + the Backtest tab · BUILT on feat/backtest (ADR-077; HC-SH-128, HC-TR-184..185; GAPS #93) · B9b next: replay an expiry's chain (daily from chain_eod, 5-minute over the last week)
