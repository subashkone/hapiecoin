// Assembles shell-head.html + core.js + parts/*.html (sorted) + features screen into coingreeks-clone.html
const fs = require('fs');
...
let featHtml = '<section class="screen" data-route="/features" data-title="Feature Inventory"><div data-cg="app-header"></div><div class="container py-8"><h1>Feature inventory</h1><p class="text-muted mt-2">Every screen, control and behaviour found on coingreeks.com, and how the clone reproduces it. ' + features.length + ' features across ' + Object.keys(byScreen).length + ' screens. Status: <span class="badge badge-success">working</span> = interactive in this mockup · <span class="badge badge-secondary">static</span> = rendered with mock data only · <span class="badge badge-warning">inferred</span> = behaviour reconstructed from the bundle, not observed.</p><div class="mt-4 flex gap-2 items-center"><input class="input" style="max-width:320px" placeholder="Filter features…" id="feat-filter"><span class="text-muted text-xs" id="feat-count"></span></div><div id="feat-list" class="mt-4">';
...
const out = head + '\n<script>\n' + core + '\n</script>\n<div id="cg-app">\n' + body + '\n' + featHtml + '\n' + notFound + '\n</div>\n';
fs.writeFileSync(path.join(dir, 'coingreeks-clone.html'), out);
fs.writeFileSync(path.join(dir, 'coingreeks-clone.local.html'), '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><style>html{color-scheme:light}body{margin:0;font:14px system-ui,sans-serif;background:#fafafa}img{max-width:100%}[hidden]{display:none!important}</style></head><body>' + out + '</body></html>');
