...
describe("HC-TR-174 the key is resolved once and never guessed", () => {
  it("two keys and none named: positions, positions-exit, place and Trade All are refused with the count; a foreign account is refused", async () => {
    ...
    const refused = await t.request(`/v1/strategies/${s.id}/live/place`, { cookie, json: { confirm: "LIVE", brokerId: SEED.brokerId, idempotencyKey: "acct-place-none-01", expected: {} } });
    expect(refused.status).toBe(409);
    expect((await json<{ message: string }>(refused)).message).toContain("2 accounts connected · pick one");
    expect((await t.request(`/v1/strategies/${s.id}/live/place`, { cookie, json: { confirm: "LIVE", brokerId: SEED.brokerId, accountId: "crd_nope", idempotencyKey: "acct-place-foreign-01", expected: {} } })).status).toBe(409);
  ...
  it("live on Sub 1: the entry, a retry, a sync, a leg close, close-all and a fired rule all go through the Sub 1 key", async () => {
    ...
    const placed = await json<Strategy>(await t.request(`/v1/strategies/${s.id}/live/place`, { cookie, json: { confirm: "LIVE", brokerId: SEED.brokerId, accountId: sub, idempotencyKey: "acct-place-sub-01", expected: {} } }));
    ...
    expect((await t.request(`/v1/strategies/${s.id}/live/retry`, { json: { confirm: "LIVE" }, cookie })).status).toBe(200); // nothing to retry: the key was opened all the same
    ...
    expect((await t.request(`/v1/strategies/${s2.id}/live/place`, { cookie, json: { confirm: "LIVE", brokerId: SEED.brokerId, accountId: sub, idempotencyKey: "acct-place-sub-02", expected: {} } })).status).toBe(200);
  ...
  it("Trade All: a paper strategy started on Sub 1 stays on Sub 1 even when the batch picks Main; one without an account takes the batch's", async () => {
    ...
    const r = await json<{ placed: string[]; failed: unknown }>(await t.request("/v1/strategies/live/batch", { cookie, json: { confirm: "LIVE", ids: [onSub.id, onNone.id], brokerId: SEED.brokerId, accountId: main, idempotencyKey: "acct-batch-mixed-01" } }));
...
describe("HC-TR-173 a second key stamps the running strategies with the first; the delete is refused while a live strategy names a key", () => {
  it("stamping: paper and live rows on that exchange with no account, not drafts or archived ones", async () => {
    ...
    expect((await t.request(`/v1/strategies/${live.id}/live/place`, { cookie: other, json: { confirm: "LIVE", brokerId: SEED.brokerId, idempotencyKey: "acct-stamp-live-01", expected: {} } })).status).toBe(200);
