...
export interface TradePreviewProps {
  ...
  /** Place; `confirm` is the word as typed (live), sent to the API as the interlock (HC-TR-186). */
  onTrade: (confirm: string) => void;
}
...
export function TradePreviewDialog(p: TradePreviewProps) {
  ...
  const wordOk = p.mode !== "live" || isLiveConfirm(word);
  const canTrade = !(p.venue !== null && p.venue !== undefined && !p.venue.ok) && wordOk && !p.busy;
      ...
      <DialogContent className="sm:max-w-[640px]" data-testid="trade-preview" data-tour="trade-preview">
            ...
            <div className="mt-3 rounded border border-border p-2 text-2xs" data-testid="venue-preview" data-ok={p.venue.ok}>
              ...
              <div className="mt-1 flex flex-wrap gap-x-3">
                ...
                {p.venue.marginRequired !== null ? (
                  <span title="HapieCoin's estimate of the margin the exchange will hold for the short legs and any future, plus the premium paid; the whole order set is refused before the first order when the wallet has less free (ADR-091)">
                    Margin needed · est. <b className={cn("num", p.venue.available !== null && Number(p.venue.marginRequired) > Number(p.venue.available) && "text-loss")} data-testid="venue-margin-required">{p.venue.marginRequired} {p.venue.availableAsset ?? "USD"}</b>
                  </span>
                ) : null}
        ...
        <DialogFooter>
          ...
          {mindful && pauseLeft > 0 ? (
            ...
            <Button onClick={() => p.onTrade(word)} loading={p.busy} disabled={!canTrade && !p.busy} variant={p.mode === "live" ? "destructive" : "primary"} data-testid="trade-now" data-tour="trade-confirm-button">
