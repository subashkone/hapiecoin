                ...
                <div className={cn("mt-3 rounded border p-2 text-2xs", venue ? (venue.ok ? "border-profit/40" : "border-loss/60") : "border-border")} data-testid="adjust-venue" data-ok={venue ? String(venue.ok) : "pending"}>
                  ...
                  {venue ? (
                    <div className="mt-1 flex flex-wrap gap-x-4 gap-y-0.5 text-muted-foreground">
                      ...
                      {venue.marginRequired !== null ? <span title="Estimated margin for the short legs and any future, plus the premium paid (ADR-091, ADR-095)">margin needed · est. <b className={cn("num", venue.available !== null && Number(venue.marginRequired) > Number(venue.available) ? "text-loss" : "text-foreground")} data-testid="venue-margin-required">{venue.marginRequired} {venue.availableAsset ?? "USD"}</b></span> : null}
