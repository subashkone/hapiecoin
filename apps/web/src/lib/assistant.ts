...
export const ANSWERS: readonly Answer[] = [
  ...
  { match: /wizard|which strategy|what (?:should|do) i (?:trade|buy|sell)|recommend|suggest a strateg/i, text: "Open Builder → Wizard (or press W): pick your view (bullish, bearish, neutral, volatile), the move you expect and the expiry, and it ranks the defined-risk templates that profit most per unit of worst case if you are right, priced at the live chain. Use this loads the legs into the Builder with the payoff opened on your target." },
  { match: /template|iron|condor|straddle|strangle|spread|butterfly|lizard|covered|collar|protective|perp|diagonal|calendar|ladder|reversal|ratio|gut|strip|strap/i, text: `The templates strip above the Builder holds ${TEMPLATE_COUNT} ready-made setups grouped by bullish, bearish, neutral and others. Click one to load its legs at the current ATM strike, then edit strikes, expiry and lots before trading. Bull Call Spread, Iron Condor, Straddle, Strangle, Butterfly and Jade Lizard are all there. Not sure which? The Wizard tab (W) ranks the defined-risk ones for your view, move and date.` },
