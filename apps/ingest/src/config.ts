...
export const IngestEnv = z.object({
  ...
  HYPERLIQUID_URL: z.url({ protocol: /^https?$/ }).default("https://api.hyperliquid.xyz"),
  /** Undocumented but public; the whale job falls back to WHALE_WALLETS when it fails (ADR-043). */
  HYPERLIQUID_LEADERBOARD_URL: z.url({ protocol: /^https?$/ }).default("https://stats-data.hyperliquid.xyz/Mainnet/leaderboard"),
  /** Extra wallets to watch (comma-separated 0x addresses); always polled. */
  WHALE_WALLETS: z
    .string()
    .default("")
    .transform((s) => s.split(",").map((x) => x.trim().toLowerCase()).filter(Boolean))
    .pipe(z.array(z.string().regex(/^0x[0-9a-f]{40}$/, "wallet must be a 0x address"))),
  /** How many leaderboard wallets to scan for open positions (weight 2 each against Hyperliquid's 1200/min). */
  WHALE_CANDIDATES: z.coerce.number().int().min(10).max(400).default(200),
  WHALE_ALERT_USD: z.coerce.number().min(10_000).default(1_000_000),
  WALL_USD: z.coerce.number().min(10_000).default(1_000_000),
  ...
  WHALES_REFRESH_MS: z.coerce.number().int().min(15_000).default(60_000),
