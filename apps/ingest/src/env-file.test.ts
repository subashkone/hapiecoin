...
describe("[INGEST] repo-root .env loading", () => {
  it("resolves three levels up from the module", () => {
    const src = pathToFileURL(join(tmpdir(), "repo", "apps", "ingest", "src", "main.ts")).href;
    const dist = pathToFileURL(join(tmpdir(), "repo", "apps", "ingest", "dist", "main.js")).href;
  ...
  it("loads the file once, keeps shell values on conflict, skips under NODE_ENV=test and when the file is missing", () => {
    ...
    const moduleUrl = pathToFileURL(join(root, "apps", "ingest", "src", "main.ts")).href;
    ...
    expect(loadRepoEnv(moduleUrl, { ...process.env, NODE_ENV: "development", HAPIE_TEST_UNDEF: undefined })).toBe(true);
