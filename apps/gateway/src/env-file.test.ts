...
describe("[GATEWAY] repo-root .env loading", () => {
  it("resolves three levels up from the module", () => {
    const src = pathToFileURL(join(tmpdir(), "repo", "apps", "gateway", "src", "main.ts")).href;
    const dist = pathToFileURL(join(tmpdir(), "repo", "apps", "gateway", "dist", "main.js")).href;
  ...
  it("loads the file once, keeps shell values on conflict, skips under NODE_ENV=test and when the file is missing", () => {
    ...
    const moduleUrl = pathToFileURL(join(root, "apps", "gateway", "src", "main.ts")).href;
