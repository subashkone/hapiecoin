...
describe("HC-SH-133 the gateway error sink speaks the Sentry envelope protocol without an SDK (ADR-081)", () => {
  ...
  it("redacts the same keys the logger does, cuts cycles, keeps dates and renders errors as name + message", () => {
    for (const key of ["apiKey", "api_secret", "otp", "password", "cookie", "authorization", "token", "signature"]) expect(isSecretKey(key)).toBe(true);
    for (const key of ["apiKeyMasked", "email", "requestId"]) expect(isSecretKey(key)).toBe(false);
